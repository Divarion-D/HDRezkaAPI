...
class Download:
    def __init__(self):
        ...
        search_request = requests.get(
            f"{API_URL}content/search/?query={search_title}&page=1"
        ).json()
        ...
        detail_info_request = requests.get(
            f"{API_URL}content/details/?url={self.url}"
        ).json()
    ...
    def Download_Season(self, season_id, translation_id):
        ...
        series_list = requests.get(
            f"{API_URL}content/tv_series/seasons/?url={self.url}&translation_id={translation_id}"
        ).json()
        for i in range(1, len(series_list["episodes"][str(season_id)])+1):
            episode_source = requests.get(
                f"{API_URL}content/tv_series/videos/?url={self.url}&translation_id={translation_id}&season_id={season_id}&episode_id={i}"
            ).json()
